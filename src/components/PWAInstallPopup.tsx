...
const PWAInstallPopup = () => {
  ...
  const handleInstall = async () => {
    ...
    if (isSafari) {
      // Safari instructions
      alert(t('pwa.safari.instructions'));
      setShowPopup(false);
      return;
    }

    if (!deferredPrompt) {
      alert(t('pwa.not.available'));
      setShowPopup(false);
      return;
    }
          ...
          <div className="relative overflow-hidden rounded-2xl p-4 sm:p-6 text-white shadow-2xl"
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 sm:gap-3 mb-3">
                ...
                <div>
                  <h3 className="font-bold text-base sm:text-lg text-white">{t('pwa.install.title')}</h3>
                  <p className="text-white/80 text-xs sm:text-sm">{t('pwa.install.subtitle')}</p>
                </div>
              </div>
