...
const LanguageSelector = ({ variant = 'desktop', className = '' }: LanguageSelectorProps) => {
  ...
  // Get the actual detected language or fallback to English
  const getFallbackLanguage = () => {
    // If we have a language but it's not in our list, try to find the closest match
    if (language && !currentLanguage) {
      // Handle language codes with country codes (e.g., 'pt-BR' -> 'pt')
      const baseLanguage = language.split('-')[0];
      const matchedLanguage = languages.find(lang => lang.value === baseLanguage);
    ...
    return languages.find(lang => lang.value === 'en') || languages[0];
  };

  const displayLanguage = currentLanguage || getFallbackLanguage();
  
  // Show loading state or fallback if not ready
  if (isLoading || !currentLanguage) {
    
    if (variant === 'compact') {
      return (
        <select 
          value={language || displayLanguage.value} 
    ...
    if (variant === 'mobile') {
      return (
        <div className="relative">
          ...
          >
            <img src={`https://flagcdn.com/w80/${displayLanguage.flag.toLowerCase()}.png`} alt={displayLanguage.label} className="w-5 h-4" />
                ...
                {languages.map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm font-medium transition-colors duration-200 hover:bg-blue-50 ${
                      (language || displayLanguage.value) === lang.value ? 'bg-blue-50 text-blue-700' : 'text-gray-700'
                    }`}
                  >
              ...
              {languages.map((lang) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-left text-sm font-medium transition-colors duration-200 hover:bg-blue-50 ${
                    (language || displayLanguage.value) === lang.value 
  ...
  // Normal rendering when language is properly loaded
  if (variant === 'compact') {
    return (
      <select 
        value={language} 
